...
logMinusH = log(1 - h);
firstTerm = ybin .* logH;
secondTerm = (1-ybin) .* logMinusH;
jMatrix = firstTerm + secondTerm;
J = -1/m * sum(sum(jMatrix));

% regularize the cost function
costLambda= sum(sum(theta1(:, 2:end) .^2)) + sum(sum(theta2(:, 2:end) .^2));
costLambda *= lambda / (2*m);
J += costLambda;
